...
// AstraZeneca brand-inspired colors
const COLORS = [
  "#7c3a5c",  // Primary purple
  "#5a2a42",  // Dark purple
  "#a855f7",  // Violet
  "#3b82f6",  // Blue
  "#10b981",  // Green
...
const VisualizationPanel: React.FC<VisualizationPanelProps> = ({ config }) => {
  ...
  const transformData = () => {
    ...
    // For single series or when all series have same x values
    const allXValues = new Set<string>();
    config.series.forEach((s) => {
      s.data.forEach((d) => allXValues.add(String(d.x)));
    });
  ...
  // Prepare pie data from bar data
  const getPieData = () => {
    if (config.series.length === 0) return [];

    // For single series, use x values as categories
    if (config.series.length === 1) {
      return config.series[0].data.map((d, i) => ({
        name: String(d.x),
        value: d.y,
    ...
    // For multiple series, aggregate by series name
    return config.series.map((s, i) => ({
      name: s.name,
      value: s.data.reduce((sum, d) => sum + d.y, 0),
      fill: COLORS[i % COLORS.length],
    }));
  };
    ...
    <ResponsiveContainer width="100%" height={320}>
      <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
        ...
        <Legend
          ...
          iconType="circle"
          iconSize={10}
          formatter={(value) => <span style={{ color: '#374151', fontWeight: 500, fontSize: 13 }}>{value}</span>}
        />
          ...
          <Tooltip
            ...
            formatter={(value: number, name: string) => [
              <span key={name}>
                ...
                {config.yLabel && <span style={{ color: '#6b7280', marginLeft: 4 }}>{config.yLabel}</span>}
                <span style={{ color: '#9ca3af', marginLeft: 8 }}>({((value / total) * 100).toFixed(1)}%)</span>
              </span>,
              name
            ]}
          />
  ...
  const renderChart = () => {
    // For line charts, always show line

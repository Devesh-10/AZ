...
class AnalyticsDataRepository {
  ...
  getEnergyByType(): { energyType: string; totalValue: number }[] {
    // Aggregate by energy type - return non-overlapping categories that ADD UP
    let totalSite = 0;
    ...
    const totalNonRenewable = totalSite - totalRenewable;

    // Return: Renewable + Non-Renewable = Total
    return [
      { energyType: 'Renewable Energy', totalValue: Math.round(totalRenewable * 100) / 100 },
      { energyType: 'Non-Renewable Energy', totalValue: Math.round(totalNonRenewable * 100) / 100 },
      { energyType: 'Total Energy', totalValue: Math.round(totalSite * 100) / 100 }
    ];
  }
  ...
  getEmissionsByScope(): { scopeType: string; totalValue: number }[] {
    ...
    const totalEmissions = scope1Total + scope2MarketBased;
    ...
    return [
      ...
      { scopeType: 'Total Emissions (Scope 1+2)', totalValue: Math.round(totalEmissions * 100) / 100 }
    ];
  }

...
module "backend" {
  ...
  environment_variables = {
    ...
    S3_KNOWLEDGE_BASE_BUCKET = module.s3.bucket_id
    OPENSEARCH_ENDPOINT      = "https://${module.opensearch.domain_endpoint}"
    AWS_DEFAULT_REGION       = var.aws_region
  }

  secret_arns = {
    AZURE_AD_CLIENT_SECRET    = module.secrets_manager.secret_arns["azure-ad-client-secret"]
    LANGSMITH_API_KEY         = module.secrets_manager.secret_arns["langsmith-api-key"]
    SNOWFLAKE_CONNECTION_STRING = module.secrets_manager.secret_arns["snowflake-connection-string"]
  }
}
